...
import { Edit2, CheckCircle, AlertCircle, Files, ChevronLeft, Building2, MapPin, Users, Calendar, RefreshCw } from 'lucide-react';
...
export const RequirementsReview: React.FC<Props> = ({ files, onConfirm, onBack, agentConfig, initialData, apiKey, globalModel }) => {
  ...
  useEffect(() => {
    if (!data && files.length > 0) {
      ...
      // Set example data if no files and no initial data
      setData({
        clientName: "(예시) 대한금융그룹",
        industry: "금융/서비스",
        department: "인재개발부",
        programName: "2025 미래 리더십 역량 강화 과정",
        objectives: [
          "디지털 전환기 정체성 확립 및 리더의 역할 재정의",
          "변화 관리 및 성과 코칭 역량 고도화",
          "협력적 조직문화 구축을 위한 소통 스킬 습득"
        ],
        targetAudience: "차장/과장급 핵심 인재 50명",
        schedule: "2025년 5월 ~ 7월 (차수별 2박 3일)",
        location: "그룹 인재제일 연수원",
        modules: ["Global Leadership", "Design Thinking", "Change Management", "Coaching & Feedback"],
        specialRequests: "금융권 특화 사례 중심 워크숍 포함"
      });
      setLoading(false);
    }
  ...
  const handleReanalyze = () => {
    ...
    else alert("분석할 파일이 없습니다. 파일을 업로드해 주세요.");
  };
  ...
  const isExampleMode = files.length === 0 && !initialData;
      ...
      {isExampleMode && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3 text-amber-800 animate-pulse">
          ...
          <div className="text-sm">
            <span className="font-bold">예시 모드:</span> 업로드된 파일이 없어 가상의 데이터를 보여줍니다. 실제 제안서를 작성하려면 'RFP 분석' 단계에서 파일을 업로드해 주세요.
          </div>
        </div>
      )}
