...
interface Props {
    ...
    onSelectPinnedBid?: (bid: BidItem) => void;
    onViewAll: () => void;
...
export const Dashboard: React.FC<Props> = ({ proposals, drafts, pinnedBids = [], onNewProposal, onResumeDraft, onSelectPinnedBid, onViewAll, onUpdateDraftStatus, onDeleteDraft }) => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in-up space-y-8">
            {/* SECTION 0: PINNED BIDS FROM NARA */}
            {pinnedBids.length > 0 && (
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                            <Pin size={20} className="text-blue-600 fill-blue-600" />
                            관심 입찰 공고 (Pinned opportunities)
                        </h3>
                        <div className="text-xs text-blue-600 font-semibold bg-blue-50 px-2 py-1 rounded">
                            {pinnedBids.length}건 선점됨
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {pinnedBids.map(bid => (
                            <div key={`${bid.bidNtceNo}-${bid.bidNtceOrd}`} className="bg-white rounded-xl border border-blue-100 p-5 shadow-sm hover:shadow-md transition-all group border-t-4 border-t-blue-500">
                                ...
                                <h4 className="font-bold text-slate-900 mb-2 line-clamp-1 group-hover:text-blue-600 transition-colors" title={bid.bidNtceNm}>
                                    {bid.bidNtceNm}
                                </h4>
                                <div className="space-y-1.5 mb-4">
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-slate-500">
                                        <Calendar size={12} />
                                        <span>~ {bid.bidNtceEndDt.substring(0, 4)}-{bid.bidNtceEndDt.substring(4, 6)}-{bid.bidNtceEndDt.substring(6, 8)}</span>
                                    </div>
                                </div>
                                <button
                                    onClick={() => onSelectPinnedBid?.(bid)}
